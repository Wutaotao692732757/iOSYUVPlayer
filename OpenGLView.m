...
#pragma mark - 接口
- (void)displayYUV420pData:(AVFrame *)WTFrame width:(NSInteger)w height:(NSInteger)h
{
    //_pYuvData = data;
    ...
    @synchronized(self)
    {
        ...
        glBindTexture(GL_TEXTURE_2D, _textureYUV[TEXY]);
        GLsizei h=432; GLsizei w=768;
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w, h, GL_RED_EXT, GL_UNSIGNED_BYTE, WTFrame->data[0]);
        
        //[self debugGlError];
        
        glBindTexture(GL_TEXTURE_2D, _textureYUV[TEXU]);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w/2, h/2, GL_RED_EXT, GL_UNSIGNED_BYTE, WTFrame->data[1]);
//
//       // [self debugGlError];
//        
        glBindTexture(GL_TEXTURE_2D, _textureYUV[TEXV]);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w/2, h/2, GL_RED_EXT, GL_UNSIGNED_BYTE, WTFrame->data[2]);
